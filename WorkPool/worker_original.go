...
func NewWorker(id int, workerPool chan chan Job) Worker {
	return Worker{
		id:         id,
		//make (chan Job) tạo ra unbuffered channel
		jobQueue:   make(chan Job), //Buffer xử lý các job
		workerPool: workerPool, //1 Worker pool sẽ nhiều worker, mỗi worker xử lý nhiều job
		quitChan:   make(chan bool), //Nhận tín hiệu để dừng
	}
}
